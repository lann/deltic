...
Platform-neutral TypeScript runtime core (PLAN.md §4.3). What exists today is
the **canonical-ABI v1 value interpreter** (`src/cabi/`) and the
...
src/cabi/          v1 reference interpreter, mirroring definitions.py sections
  types.ts         provisional component value-type model (§8 descriptor-IR sketch)
...
Run tests: `deno test` (from `runtime/`). Everything passes with explicit
ignores only (`deferred_test.ts` placeholders).
