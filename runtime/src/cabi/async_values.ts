...
/**
 * Diagnostic for a handle-table entry that carries the A8 error-context brand
 * without being one of THIS copy's `ErrorContext`s (amendment A8, issue #83).
 *
...
export function errorContextTrapMessage(where: string, e: unknown): string {
  ...
  return `${where}: this error-context was minted by a DIFFERENT deltic ` +
    ...
    `${census === "" ? "" : ` (${census})`} ` +
    `(contracts/embedder-api.md amendment A8, issue #83)`;
}
...
function liftAsyncValue(
  ...
  if (holder.boundStore != null && store != null) {
    // A8: when several runtime copies are loaded, "a second store" is very
