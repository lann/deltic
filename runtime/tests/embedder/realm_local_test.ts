// A19 realm-local pill (contracts/embedder-api.md §"Realm boundaries and
...
// class this track owns, plus the A19 message-valued error-context
...
Deno.test("A19: a raw structuredClone of a Stream throws DataCloneError", async () => {
  const { stream } = Stream.create<number>();
  await assertDataCloneError(() => structuredClone(stream), "Stream");
});

Deno.test("A19: a raw structuredClone of a StreamWriter throws DataCloneError", async () => {
...
Deno.test("A19: a raw structuredClone of a Future throws DataCloneError", async () => {
  const codec = {
...
Deno.test("A19: a raw structuredClone of an embedder ErrorContext throws DataCloneError", async () => {
...
Deno.test({
  name:
    "A19: a raw structuredClone of a guest-resource wrapper throws DataCloneError",
  ignore: !ready,
  fn: async () => {
...
Deno.test("A19: the pill is buried — structuredClone descends into records/arrays", async () => {
  const { stream } = Stream.create<number>();
  await assertDataCloneError(
...
Deno.test("A19: JSON.stringify and spread of a pilled ErrorContext still work (function values omitted, not thrown)", () => {
  const ctx = new ErrorContext(new InternalErrorContext("boom"));
...
Deno.test("A19: error-context lowering accepts a hand-rolled branded carrier with a string message", () => {
  ...
  // A19: a NEW local context is minted, never "the same" one — there is
...
Deno.test("A19: error-context lowering still refuses a branded carrier WITHOUT a string message", async () => {
  const brandKey = Symbol.for("polyengine.errorContext/1");
