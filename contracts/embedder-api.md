...
and limits host ends to one in-flight operation per direction;
amendment A8 (2026-08-11) makes every cross-boundary brand a
process-global registry symbol carried by the new dependency-free
`@deltic/protocol` package — class identity is not part of the embedder
API — and adds loud multi-copy diagnostics (issue #83).** This document supersedes `descriptor-ir.md`'s interim
...
- **Recognition is by brand, not class** (amendment A8): every class above
...
- **Imports match their WIT type**: an `async func` import may be a plain
  ...
  in `@deltic/protocol` since A8; re-exported unchanged from the embedder
  surface). The marker
...
- **Foreign-copy handles are recognized and refused, loudly** (amendment
  A8). A `Stream`/`Future` (or `ErrorContext`, or a resource wrapper)
...
## Module identity and @deltic/protocol (amendment A8)
...
stream), never at instantiation. A8 removes class identity from the
contract entirely.
...
with several copies is the #83 signature (typically a pre-A8 copy), and
...
cost N compiles and N bundle payloads) and for graphs containing pre-A8
