...
| M2 | **Concurrency complete on Deno** | **Substantially done** (2026-08-08, phases 1–3g): task core, callback ABI, FACT cross-component calls (all four ABI combinations), streams/futures/error-context with full rendezvous semantics, host-side stream/future ends, cross-component cancellation, instance poisoning — **1232/1349 executing suite commands pass, zero failures, 14/31 async files fully green, sync dirs at exact baseline throughout**; wit-bindgen async roundtrip trio + producer guests green. Three upstream findings (CM-1..3) + the JSPI integration constraints recorded (contracts/intrinsics.md). **Open tail**: JSPI light-up — machinery complete and runtime-suite-green with the mode forced on; auto-detection off pending one un-traced resumption path (11 files / 16 cmds) — plus named residue: 🧵 (PLAN-deferred), wasmparser pin-drift (exits on wasmtime bump), inst-task (spec question), OTHER (3 files, ~7 cmds, untriaged). |
