...
| Safari: stable-channel JSPI status unverified | accepted | floor is explicit; JSPI unflagged on WPE 26.5, and the STP channel carries both JSPI (STP 238) and multi-memory (STP 250) — stable channel is the remaining gap ([#11](https://github.com/lann/deltic/issues/11)); callback-ABI consumers don't need it |
