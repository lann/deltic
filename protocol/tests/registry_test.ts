...
Deno.test("A8: registration is visible through the global slot", () => {
  reset();
...
Deno.test("A8: registration is idempotent per URL", () => {
  reset();
  const e = {
    url: "file:///a/mod.ts",
    runtimeVersion: "0.1.0",
...
Deno.test("A8: a foreign pre-seeded array is adopted, never replaced", () => {
  reset();
...
Deno.test("A8: entries are frozen and snapshots do not alias the slot", () => {
  reset();
  registerRuntimeCopy({
    url: "file:///a/mod.ts",
    runtimeVersion: "0.1.0",
